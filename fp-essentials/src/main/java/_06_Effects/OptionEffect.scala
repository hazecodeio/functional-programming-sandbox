...
object OptionEffectTryOut extends App {
  val opt0 = None
...
trait FishingOptionExample {
  ...
  trait flat {
    ...
    def goFishingWithFlatMapOld(bestBaitForFish: Option[String]): Option[Fish] =
      bestBaitForFish.flatMap(buyBait).flatMap(castLine).flatMap(hookFish) // Note the use of chained flatMap()

    // By me
    def goFishingWithFlatMap(bestBaitForFish: Option[String]): Option[Fish] =
      bestBaitForFish
        .flatMap(buyBait).orElse(bestBaitForFish.flatMap(makeBait)) // ToDo - Check for correctness
        .flatMap(castLine)
        .flatMap(hookFish) // Note the use of chained flatMap()

    def goFishingWithForComprehension(bestBaitForFish: Option[String]): Option[Fish] =
      for {
